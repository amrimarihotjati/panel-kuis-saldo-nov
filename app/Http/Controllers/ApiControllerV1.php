...
class ApiControllerV1 extends Controller
{
    ...
    public static function registerPlayer(Request $request)
    {
        ...
        if ($head_is_main_application == 1) {
            ...
            if ($newPlayer) {
                $playerData = Player::where('email', $data_player_email)->first();
                $primaryBadge = Badge::where('id', $playerData->badge_primary)->first();
                if (is_null($primaryBadge)) {
                    $primaryBadge = Badge::where('id', '93ced448-732b-4008-a17f-b8a89a294097')->first();
                }
                $playerData->badge_primary_name = $primaryBadge->badge_name;
                $playerData->badge_primary_icon = $primaryBadge->badge_icon;
                $deviceCreate = Device::create([
                    'device_id' => $head_device_id,
                    'player_id' => $playerData->id,
                    'player_pkg' => $playerData->player_pkg
                ]);
                return ApiFormatter::createApiSuccess('Pendaftaran berhasil dilakukan', ['Player' => $playerData]);
            } else {
                return ApiFormatter::createApiFailed('Terjadi suatu kesalahan');
            }
        } else {
    ...
    public static function getListHistoryWithdrawPlayer(Request $request) {
        ...
        // if (is_null($mHistoryWithdrawPlayerList) || $mHistoryWithdrawPlayerList->isEmpty()) {
        //     return ApiFormatter::createApiFailed('Belum ada riwayat penarikan');
        // }
    ...
    public static function getListHistoryCollectedPointPlayer(Request $request) {
        ...
        // if (is_null($listHistoryCollectedPoint) || $listHistoryCollectedPoint->isEmpty()) {
        //     return ApiFormatter::createApiFailed('Belum ada riwayat');
        // }
    ...
    public static function getListHistoryRefferal(Request $request) {
        ...
        // $mHistoryRefferaledList = RefferalPlayer::where('refferaled_from_player', $playerLoginId)
        // ->orderBy('created_at', 'desc')
        // ->orderBy('updated_at', 'desc')
        // ->get();
    ...
    public static function setCompletedQuizPlayer(Request $request) {
        ...
        $countWatchInters = $request->ads_watched_inters ?? 0;
        $countWatchRewards = $request->ads_watched_rewards ?? 0;
    ...
    public static function collectPointExternal(Request $request) {
        ...
        //     return ApiFormatter::createApiFailed('ACCESS FOR EXTERNAL COLLECTED POINT DISABLED');
        // }

        $playerEmail = $request->email;
        $pointCollected = $request->point;
        $cekPlayerExists = Player::where('email', $playerEmail)->first();
