...
@section('title', 'Analytics Withdrawal Accepted')
        ...
        <div class="card rounded rounded-4 shadow p-3">
            <div class="card-header text-dark fw-bold mt-2 d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center">
                    <div class="h5 fw-bold">Riwayat Withdrawal Accepted<br>&nbsp;</div>
                </div>
                <div class="text-end">
            ...
            <div class="card-body pt-2">
                ...
                <div class="table-responsive">
                    <table id="getDTWithdrawalRangeAccepted" class="table table-bordered table-flush table-hover d-none"
                        style="width:100%">
                        <thead>
                            <tr>
                                ...
                                <th class="align-middle">Waktu Disetujui</th>
                            </tr>
                        </thead>
                        <tbody>
                        </tbody>
    ...
    <script type="module">
        ...
        function initializeAnalyticsTableAndChart(appPackage, startDate, endDate) {
            $(document).ready(function() {
                $('#getDTWithdrawalRangeAccepted').removeClass('d-none');

                if ($.fn.DataTable.isDataTable('#getDTWithdrawalRangeAccepted')) {
                    $('#getDTWithdrawalRangeAccepted').DataTable().destroy();
                }

                var baseUrl = '/analytics-data/withdrawals/accepted/' + appPackage + '/' + startDate + '/' + endDate;

                var table = $('#getDTWithdrawalRangeAccepted').DataTable({
                    processing: true,
                    serverSide: true,
                    paging: true,
                    ajax: baseUrl,
                    ...
                    columns: [{
                        ...
                        },
                        {
                            ...
                            className: 'align-middle text-center',
                            render: function(data, type, full, meta) {
                                return '<span class="fw-bold text-muted">' + moment(full.updated_at)
                                    .format('DD/MM/YYYY') +
                                    '</span>';
                            }
                        }
        ...
        function updateChart(table) {
            ...
            table.rows().every(function(rowIdx, tableLoop, rowLoop) {
                var data = this.data();
                var formattedDate = moment(data['updated_at']).format('DD/MM/YYYY');
